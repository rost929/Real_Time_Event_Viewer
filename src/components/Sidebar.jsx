...
function Sidebar({ events, status }) {
  ...
  return (
    <div style={{ width: "300px", padding: "20px", backgroundColor: "#f0f0f0", height: "100vh", overflowY: "auto" }}>
      ...
      <select value={category} onChange={(e) => dispatch(setCategory(e.target.value))} style={filterInputStyle}>
      ...
      {category === "earthquakes" && (
        <>
          <select value={timePeriod} onChange={(e) => dispatch(setTimePeriod(e.target.value))} style={filterInputStyle}>
      ...
      {category === "volcanoes" && <>{searchInput}</>}
